...
def xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation, depth_multiplier, with_global=False):
    ...
    nn_fts_from_pts_0 = layers.dense(nn_pts_local, C_pts_fts, tag + 'nn_fts_from_pts_0', is_training)
    nn_fts_from_pts = layers.dense(nn_fts_from_pts_0, C_pts_fts, tag + 'nn_fts_from_pts', is_training)
    if fts is None:
    ...
    if with_X_transformation:
        ######################## X-transformation #########################
        X_0 = layers.conv2d(nn_pts_local, K * K, tag + 'X_0', is_training, (1, K))
        X_0_KK = tf.reshape(X_0, (N, P, K, K), name=tag + 'X_0_KK')
        X_1 = layers.depthwise_conv2d(X_0_KK, K, tag + 'X_1', is_training, (1, K))
        X_1_KK = tf.reshape(X_1, (N, P, K, K), name=tag + 'X_1_KK')
        X_2 = layers.depthwise_conv2d(X_1_KK, K, tag + 'X_2', is_training, (1, K), activation=None)
    ...
    fts_conv = layers.separable_conv2d(fts_X, C, tag + 'fts_conv', is_training, (1, K), depth_multiplier=depth_multiplier)
    fts_conv_3d = tf.squeeze(fts_conv, axis=2, name=tag + 'fts_conv_3d')

    if with_global:
        fts_global_0 = layers.dense(qrs, C // 4, tag + 'fts_global_0', is_training)
        fts_global = layers.dense(fts_global_0, C // 4, tag + 'fts_global', is_training)
...
class PointCNN:
    def __init__(self, points, features, is_training, setting):
        ...
        if features is None:
            ...
            features_hd = layers.dense(features, C_fts, 'features_hd', is_training)
        ...
        for layer_idx, layer_param in enumerate(fc_params):
            ...
            fc = layers.dense(self.fc_layers[-1], C, 'fc{:d}'.format(layer_idx), is_training)
